...
class Liver_Fibrosis_Solver(Export):
    ...
    def plot_slice_custom(self, step=-1, x_slice=None, y_slice=None, z_slice=None, save_path=None):
        ...
        im02 = axes[0, 2].imshow(Coll[x_slice, :, :], cmap='inferno', origin='lower', vmin=0, vmax=Coll_max)
        ...
        im12 = axes[1, 2].imshow(Coll[:, y_slice, :], cmap='inferno', origin='lower', vmin=0, vmax=Coll_max)
        ...
        im22 = axes[2, 2].imshow(Coll[:, :, z_slice], cmap='inferno', origin='lower', vmin=0, vmax=Coll_max)
...
if __name__ == "__main__":
    ...
    feb_path = Path("Z:/Model/init.feb")
    ...
    for i in range(0, simulation.n_steps, 1000):
        #if i > 0:  # Éviter l'état initial qui est déjà affiché
        simulation.plot_slice_custom(step=i, x_slice=5, y_slice=5, z_slice=10)
            
    simulation.plot_norms()
    
    save_to_file('Z:/Boucle_Complete/coll_final_5.txt', simulation.x, simulation.y, simulation.z, simulation.Coll_results[-1])
    save_to_file('Z:/Boucle_Complete/fib_final_5.txt', simulation.x, simulation.y, simulation.z, simulation.F_results[-1])
