...
if __name__ =='__main__':
    ...
    for ids in iteration:
        ...
        if ids == 1:
            ...
            solve.plot_norms(ids = ids)
            solve.demie_vie_cyto()
            ...
            for key, value in coll_dict.items():
                new_young_dict[key] = 1000 * coll_dict[key] + young_init
            ...
            plt.bar(positions, [len(data) for data in elmt_data])
            plt.xticks(positions, [f'{y:.3f}' for y in Y], fontsize=18, rotation=45, ha='right')
            plt.title(f'Nombre d éléments par module de young (iteration {ids})')
            plt.ylabel('Nombre d éléments')
            plt.xlabel('Module de Young')
            plt.show()
        ...
        if ids > 1:
            ...
            coll_dict = data_dict(coll_value_interpolate_path)
            for key, value in coll_dict.items():
                new_young_dict[key] = old_young_dict[key] + 1000 * coll_dict[key]
            print(new_young_dict)
            ...
            positions = range(len(Y))
            plt.bar(positions, [len(data) for data in elmt_data])
            plt.xticks(positions, [f'{y:.3f}' for y in Y], fontsize=14, rotation=45, ha='right')
            plt.title(f'Nombre d éléments par module de young (iteration {ids})')
            plt.ylabel('Nombre d éléments')
            plt.xlabel('Module de Young')
            plt.show()
